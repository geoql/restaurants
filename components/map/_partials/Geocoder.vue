<template>
  <div class="h-full w-full">
    ...
    <input
      ...
      type="text"
      name="geocoder"
      class="block h-full w-full rounded-md bg-stone-200 bg-opacity-25 text-stone-900 shadow-sm dark:bg-stone-800 sm:text-sm dark:text-stone-50"
    ...
    <div
      ...
      class="mt-1 flex flex-col overflow-auto divide-y divide-stone-100 dark:divide-stone-500"
    >
      <div
        v-for="({ label, lng, lat, zoom }, index) in state.results"
        :key="index"
        class="flex-1 cursor-pointer select-none truncate bg-stone-200 bg-opacity-50 px-1 dark:bg-stone-800 hover:bg-opacity-100"
        :title="label"
